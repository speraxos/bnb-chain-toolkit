  ...
  server.tool(
    ...
    async ({ address }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Failed to retrieve BTC balance: ${error.message}` }]
        }
      }
    }
  )
        ...
        const txList = txs.txs
          .map((tx: any) => {
            const fromAmount = tx.from[0]?.amount
              ? `${formatBaseAmount(tx.from[0].amount)} BTC`
              : "Unknown"
            const toAmount = tx.to[0]?.amount
              ? `${formatBaseAmount(tx.to[0].amount)} BTC`
              : "Unknown"
...
Date: ${new Date(tx.date).toLocaleString()}`
        ...
        return {
          content: [
            { type: "text" as const, text: `Failed to retrieve BTC transaction history: ${error.message}` }
          ]
        }
      }
    }
  ...
  server.tool(
    ...
    async ({ address }) => {
      try {
        ...
        return {
          content: [
            { type: "text" as const, text: `Error validating Bitcoin address: ${error.message}` }
          ]
        }
      }
    }
  ...
  Average: ${fees.average}`
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Failed to retrieve Bitcoin network information: ${error.message}`
            }
          ]
        }
      }
