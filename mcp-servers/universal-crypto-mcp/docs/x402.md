...
<Tabs>
  ...
  <Tab title="Python">
    ```python theme={null}
    from x402 import x402Server

    server = x402Server()

    async def on_payment_required(context):
        # Check for valid API key before requiring payment
        auth_header = context.request.headers.get("Authorization", "")
        if auth_header.startswith("Bearer "):
            api_key = auth_header[7:]
            if await validate_api_key(api_key):
                return {"bypass": True}
        # Return None to continue to payment flow

    server.on_payment_required(on_payment_required)
    ```
  </Tab>
  ...
  <Tab title="Go">
    ...
    server := x402.Newx402Server()

    server.OnPaymentRequired(func(ctx x402.PaymentRequiredContext) (*x402.PaymentRequiredResult, error) {
        // Check for valid API key before requiring payment
        authHeader := ctx.Request.Header.Get("Authorization")
        if strings.HasPrefix(authHeader, "Bearer ") {
            apiKey := authHeader[7:]
            if validateAPIKey(apiKey) {
                return &x402.PaymentRequiredResult{Bypass: true}, nil
            }
        }
        // Return nil to continue to payment flow
        return nil, nil
    })
    ```
  </Tab>
...
<Tabs>
  ...
  <Tab title="Python">
    ```python theme={null}
    from x402 import x402Client, x402HTTPClient
    import os
    ...
    async def on_payment_required(context):
        ...
        # Return None to proceed with payment
  ...
  <Tab title="Go">
    ...
    client := x402.Newx402Client()
    httpClient := x402.Newx402HTTPClient(client)

    httpClient.OnPaymentRequired(func(ctx x402.PaymentRequiredContext) (*x402.PaymentRequiredResult, error) {
        // Try API key authentication first
        apiKey := os.Getenv("API_KEY")
        if apiKey != "" {
            return &x402.PaymentRequiredResult{
                Headers: map[string]string{
                    "Authorization": "Bearer " + apiKey,
                },
            }, nil
        }
        // Return nil to proceed with payment
        return nil, nil
    })
    ```
  </Tab>
